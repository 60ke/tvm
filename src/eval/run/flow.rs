...
pub fn sload<M: Memory + Default, P: Patch>(state: &mut State<M, P>) {
    ...
    push!(state, value);
    println!("state.stack.push value:{:?} sputnikvm/src/eval/run/flow.rs:12",value);
}
...
pub fn sstore<M: Memory + Default, P: Patch>(state: &mut State<M, P>) {
    ...
    println!("sstore address:{:?} index:{:?} value:{:?} sputnikvm/src/eval/run/flow.rs:18", state.context.address,index,value);
}
