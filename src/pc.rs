    ...
    ($t: tt) => {
        impl<'a, P: Patch> $t<'a, P> {
            ...
            pub fn peek_opcode(&self) -> Result<Opcode, OnChainError> {
                println!("当前栈的位置{}(sputnikvm/src/pc.rs:157){:?}", *self.position,self.code);
            ...
            pub fn peek(&self) -> Result<Instruction, OnChainError> {
                ...
                Ok(match opcode {
                    ...
                    Opcode::SLOAD => {println!{"sload 被调用！"}; Instruction::SLOAD},
                    Opcode::SSTORE => {println!("sstore 被调用(sputnikvm/src/pc.rs:221)");Instruction::SSTORE},
                    Opcode::JUMP => Instruction::JUMP,
                    Opcode::JUMPI => Instruction::JUMPI,
                    Opcode::PC => {println!("pc 被调用");Instruction::PC},
                    ...
                    Opcode::PUSH(v) => {
                        ...
                        println!("调用push，push{:#?}sputnikvm/src/pc.rs:231",param);
